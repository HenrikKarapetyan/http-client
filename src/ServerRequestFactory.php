...
abstract class ServerRequestFactory
{
     ...
     * @param array $server
     * @param MessageInterface $request
     * @return void Array with two members, host and port, at indices 0 and 1, respectively
    ...
    {
        self::marshalHostAndPortFromHeaders($accumulator, $server, $request->getHeaders());
        return;
    }
     ...
     *
     * @param array $upl_files
     * @return UploadedFileInterface[]
     */
    private static function normalizeNestedFileSpec(array $upl_files)
    {
        $files = [];
        foreach (array_keys($upl_files['tmp_name']) as $key) {
            $spec = [
                'tmp_name' => $upl_files['tmp_name'][$key],
                'size' => $upl_files['size'][$key],
                'error' => $upl_files['error'][$key],
                'name' => $upl_files['name'][$key],
                'type' => $upl_files['type'][$key],
            ];
